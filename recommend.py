...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    items = [[find_closest(restaurant_location(restaurant), centroids), restaurant] for restaurant in restaurants]
    return group_by_first(items)
    # END Question 4
...
def find_centroid(cluster):
    ...
    for x in cluster:
        latitudes = latitudes + [restaurant_location(x)[0]]
        longitudes = longitudes + [restaurant_location(x)[1]]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants: 
        if restaurant in reviewed:
            compiled[restaurant_name(restaurant)] = user_rating(user,restaurant_name(restaurant))
        else:
            compiled[restaurant_name(restaurant)] = predictor(restaurant)
    return compiled
